  ...
  return (
    <section className="run-summary" aria-label="Assessment run summary">
      <div className="run-summary__hero">
        <div>
          <p className="run-summary__eyebrow">Loaded assessment graph</p>
      ...
      <div className="run-summary__cards">
        ...
        <article className="run-card">
          <span className="run-card__label">Findings</span>
          <strong>{graph.findings.length}</strong>
          <span>{graph.coverage.gaps.length} explicit coverage gaps</span>
        </article>
        <article className="run-card">
          <span className="run-card__label">Graph</span>
          <strong>{graph.nodes.length + graph.intents.length} nodes</strong>
          <span>{graph.edges.length} edges, schema {graph.version}</span>
        </article>
      </div>
      ...
      <div className="severity-strip" aria-label="Findings by severity">
        {SEVERITIES.map((sev) => (
          <span key={sev} className={`severity-pill severity-pill--${sev.toLowerCase()}`}>
            {sev} <strong>{graph.summary.bySeverity[sev] ?? 0}</strong>
          </span>
        ))}
      </div>
    </section>
